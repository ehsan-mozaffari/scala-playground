...
class LoopsSpec extends AnyFlatSpec with should.Matchers {
  ...
  "for" should
    ...
    var separatedResult = ""
    val unitResult: Unit = for (f <- fruit)
      separatedResult += s"$f,"

    val result: Seq[String] = for (f <- fruit) yield f

    val complexResult: Seq[String] = for {
      f <- fruit
      g <- fruit
    } yield s"($f, $g)"

    val complexResultWithGuard: Seq[String] = for {
      f <- fruit
    ...
    complexResult.mkString(",") shouldEqual "(apples, apples),(apples, bananas),(apples, cherries),(bananas, apples),(bananas, bananas),(bananas, cherries),(cherries, apples),(cherries, bananas),(cherries, cherries)"
