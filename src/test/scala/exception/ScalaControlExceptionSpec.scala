...
class ScalaControlExceptionSpec extends AnyFlatSpec with should.Matchers {
  ...
  "catching" should
    ...
    catching(exceptions: _*).either(1 / 0) shouldBe an[
      Left[NumberFormatException, Int]
    ]
    ...
    an[NumberFormatException] shouldBe thrownBy(
      catching(exceptions: _*).opt(testNonConvertibleStringToInt.toInt)
    )
  }
  ...
  "catchSpecificExceptions" should
    ...
    val result = catchSpecificExceptions(classOf[ArithmeticException]) {
      val list = List(1, 2, 3)
      (list.map(_ + 1) sum) / 2
    }

    val noResultDividedByZero = catchSpecificExceptions(classOf[ArithmeticException]) {
      val list = List(1, 2, 3)
      (list.map(_ + 1) sum) / 0
    }
